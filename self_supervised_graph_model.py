...
# --- 1. Hybrid GNN / Autoencoder Model Definition ---
...
def main(args):
    ...
    for directory in abn_directories:
        ...
        for epoch in range(1, N_EPOCHS + 1):
            ...
            if args.decorrelation_strength > 0 and args.use_neighbors:
                ...
                # We penalize the mean squared cosine similarity to encourage orthogonality
                similarities = F.cosine_similarity(z_source, z_target, dim=1, eps=1e-8)
                decorrelation_loss = torch.mean(similarities.pow(2))
        ...
        if args.use_neighbors:
            latent_key = f'X_{args.model_type.lower()}_latent_neighbors'
        else:
            latent_key = f'X_{args.model_type.lower()}_latent_no_neighbors'
    ...
    if args.use_neighbors:
        model_path = f'global_{args.model_type.lower()}_model_with_neighbors.pt'
    else:
        model_path = f'global_{args.model_type.lower()}_model_no_neighbors.pt'
        
    torch.save(model.state_dict(), model_path)
    print(f"Saved globally trained {args.model_type} model to '{model_path}'.")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Self-supervised Graph Neural Network for spatial transcriptomics.")
    parser.add_argument(
        '--model_type',
        type=str,
        default='GAT',
        choices=['GAT', 'GCN'],
        help="Type of GNN model to use. (default: GAT)"
    )
    parser.add_argument(
        '--use_neighbors',
